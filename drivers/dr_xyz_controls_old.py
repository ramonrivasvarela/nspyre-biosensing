...
class NIDAQMotionController(Instrument):
...
class XYZSetup(Instrument):
    ...
    def x_move(self, val):
        self.daq_controller.move({'x': Q_(val, 'um'), 'y': self.y(), 'z': self.z()})

    def y(self):
        return self.daq_controller.position['y']

    def y_move(self, val):
        self.daq_controller.move({'x': self.x(), 'y': Q_(val, 'um'), 'z': self.z()})

    def z(self):
        return self.daq_controller.position['z']

    def z_move(self, val):
        self.daq_controller.move({'x': self.x(), 'y': self.y(), 'z': Q_(val, 'um')})
