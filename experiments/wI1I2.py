...
class wI1I2Spyrelet(WFSpyrelet):
    ...
    def __enter__(self):
        ...
        _logger.info('Created wI1I2 instance.')


    def __exit__(self):
        """Perform experiment teardown."""
        _logger.info('Destroyed wI1I2 instance.')
    ...
    def init_sg(self, mgr, rf_amplitude, sideband):
        ...
        mgr.sg.set_FM_mod_dev(sideband)
        return
    
    def init_seq(self, mgr, alt_label):
        self.main_seq=mgr.Pulser.WF_prep_I1I2(self.label, self.exp_time, self.readout_time, trig = self.trigger_time, buff = self.buffer_time) 
        if alt_label:
            self.alt_seq=mgr.Pulser.WF_prep_I1I2(self.label_inv, self.exp_time, self.readout_time, trig = self.trigger_time, buff = self.buffer_time)
    ...
    def initialize(self, mgr, exp_time, readout_time, label, frequencies, cooler, 
                   cam_trigger, rf_amplitude, sideband, ROI_xy, alt_label, trackpy_params, 
                   data_path, save_image, data_download, verbose, window_params, Misc): 
        ...
        ## Routine
        self.freqs = np.linspace(eval(frequencies)[0], eval(frequencies)[1], eval(frequencies)[2], endpoint = True)
        ## Trackpy 
        self.trackpy_params = eval(trackpy_params)
        #### SG init ########################################################################
        self.init_sg(mgr, rf_amplitude, sideband) # init signal generator
        #### Cam init ########################################################################
        self.init_camera(mgr, cam_trigger) # self.sdk | self.sdk_codes | camera initialized and ready to acquire
        #### Seq init ########################################################################
        self.init_seq(mgr, alt_label) # self.seq_key | self.alt_key (if alt_label) | self.gain_key | sequences initialized and ready to acquire
        #### Experiment Specific Init #####################################################################
        #### Cooldown if necessary ########################################################################
        do_cool, temp = eval(cooler)
        if do_cool: self.cool_loop(mgr, temp)
        #### Checks for validity ################################################
    ...
    def init_main_loop(self, mgr): # TEMPLATE
        ## Prepare camera
        mgr.Camera.set_number_kinetics(len(self.label))
        ## Prepare SG, ps
        mgr.Pulser.set_state([6], 0.0, 0.0) #set pulser to safe output
    ...
    def main(self, exp_time, readout_time, sweeps, label, frequencies, gain, gain_setting, cooler, 
                   ...
                   focus_bool, fourier_filter, data_path, save_image, data_download, verbose, window_params, shutdown, dataset,Misc): 
        params={'exp_time': exp_time, 'readout_time': readout_time, 'sweeps': sweeps, 'label': label, 'frequencies': frequencies, 'gain': gain, 'gain_setting': gain_setting, 'cooler': cooler, 
                   'cam_trigger': cam_trigger, 'rf_amplitude': rf_amplitude, 'sideband': sideband, 'ROI_xy': ROI_xy, 'alt_label': alt_label, 'alt_sleep_time': alt_sleep_time, 'trackpy_params': trackpy_params, 
                   'focus_bool': focus_bool, 'fourier_filter': fourier_filter, 'data_path': data_path, 'save_image': save_image, 'data_download': data_download, 'verbose': verbose, 'window_params': window_params, 'shutdown': shutdown, 'Misc': Misc}
        ...
        with InstrumentManager() as mgr, DataSource(dataset) as data_source:
            self.initialize(mgr, exp_time, readout_time, label, frequencies, cooler, 
                   cam_trigger, rf_amplitude, sideband, ROI_xy, alt_label, trackpy_params, 
                   data_path, save_image, data_download, verbose, window_params, Misc)
            self.gain, max_value, representative_img = self.init_gain(mgr, gain, gain_setting, self.Misc.get('ideal_pixel_range', (2.1e4,2.9e4)), 
    ...
    def save_config(self, data_download, sweeps, alt_label, rf_amplitude, frequencies, gain, exp_time, readout_time, ROI_xy, sideband):
        ...
        return total_time
    ###########################################################################################################################
    def finalize(self, mgr, exp_time, readout_time, sweeps, frequencies, gain, 
                   rf_amplitude, sideband, ROI_xy, alt_label, 
                   data_download, shutdown):  
        #### SG off ###############################################################################################
