...
## NSpyre
from nspyre import InstrumentManager, nspyre_init_logger
...
# Instrument manager used by this module. It is opened in WFSpyrelet.__enter__
# and closed in WFSpyrelet.__exit__.
...
class WFSpyrelet():
    ...
    if True:
        ...
        def set_pulse_self_params(self, exp_time, readout_time, trigger_time, buffer_time):
            self.exp_time = exp_time * 1e9 # nanoseconds
            self.readout_time = readout_time * 1e9 # nanoseconds
        ...
        def set_ND_self_params(self, mgr, ROI_xy, r_track, r_targ):
            ...
            # DAQ stage controller (hosted as 'DAQcontrol' in inserv.py)
            self.z_pos = float(mgr.DAQcontrol.position['z'])
            return
        ...
        def init_sg(self, mgr, rf_amplitude): # TEMPLATE
        ...
        def init_camera(self, mgr, cam_trigger):
            ...
            ret = mgr.Camera.initialize()
            print("Camera initialization returned {}".format(ret))
            mgr.Camera.set_VS_speed(3)
            mgr.Camera.set_HS_speed(0)
            ...
            if cam_trigger == 'EXTERNAL_EXPOSURE':
                ...
                mgr.Camera.set_exposure_time(self.exp_time / 1e9)
                mgr.Camera.set_frame_transfer_mode("on")

            (ret, xpixels, ypixels) = mgr.Camera.get_detector()
            mgr.Camera.set_image(xpixels, ypixels) # defining image
            #self.sdk.SetBaselineClamp(1)
            mgr.Camera.set_shutter("open") # Open Shutter
            return
        ## Process Functions (functional)
        def cool_loop(self, mgr, temp):
            ret = mgr.Camera.set_temperature(temp)
            print(f"Function set_temperature returned {ret} target temperature {temp}")
            ...
            ret = mgr.Camera.cooler_on()
            ct = 0
            inp = ''
            while ret != atmcd_errors.Error_Codes.DRV_TEMP_STABILIZED and inp != 'c':
                ct = ct%3
                (ret, temperature) = mgr.Camera.get_temperature_status()
                if inp != 'A':
        ...
        def init_gain(self, mgr, gain, gain_setting, ideal_pixel_range, max_checks, max_gain, gain_wait_time, img_cycles_no_gain_optimization, save = 'no_save'):
            '''
            ...
            if gain_setting == 'optimize':
                self.gain = gain
                mgr.Camera.set_emccdgain(self.gain)
                ...
                current_gain = mgr.Camera.get_emccdgain()[1]
                ...
                try:
                    ...
                    try:
                        ...
                        if blob_log is None:
                            ...
                            blob_log = skimage_feature.blob_log
