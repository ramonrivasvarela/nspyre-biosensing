...
class ODMRCenter:
    ...
    def main(self, runs, initial_odmr, odmr_span, sweep_time, PID, probe_time, clock_time, laser_pause, timeout_counter, dataset):
        ...
        with InstrumentManager() as mgr, DataSource(dataset) as ds:
            ...
            while counter<timeout_counter:
                ...
                left = mgr.DAQcontrol.read_to_data_array()
                mgr.sg.set_mod_toggle(False)
                mgr.sg.set_frequency(odmr_freq+odmr_span/2)
                mgr.DAQcontrol.start_counter()
                mgr.sg.set_mod_toggle(True)
                mgr.Pulser.stream_sequence(self.sequence, runs)
                right = mgr.DAQcontrol.read_to_data_array()
    ...
    def initialize(self, mgr, runs,odmr_span, sweep_time, probe_time, clock_time, laser_pause):
        ...
        sg_span=2*(odmr_span/sweep_time)*(sweep_time+laser_pause)
        self.sequence=self.create_sequence(mgr, clock_time, probe_time, n_points)
        mgr.sg.continuous_sweep(sg_span, 2*(sweep_time+laser_pause))
        mgr.DAQcontrol.create_counter()
        mgr.DAQcontrol.prepare_counting(2/probe_time, 2*n_points*runs)

        return
    
    def process_data(self, runs, n_points, data, left):
        # Reshape data to separate runs
        points_per_run = 2 * n_points + 1  # (n_points+1) background + n_points signal
        ...
        if left:
            ...
            background_raw = data_reshaped[:, n_points+1:2*n_points+1]  # Next n_points are signal
    ...
    def create_sequence(self, mgr, clock_time, probe_time, laser_pause, n_points, left):
        ...
        if left:
            laser=[(laser_pause_ns,0)] + laser
            clock=[(laser_pause_ns,0)] + clock
        else:
            laser= laser+[(laser_pause_ns,0)]
            clock= clock+[(laser_pause_ns,0)]
