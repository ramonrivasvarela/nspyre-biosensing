...
class ODMRCenter:
    ...
    def initialize(self, mgr, runs,odmr_span, sweep_time, probe_time, clock_time, laser_pause, mode, rf_amplitude):
        ...
        self.sequence_left=self.create_sequence(mgr, clock_time, probe_time, laser_pause,left=True)

        self.sequence_right=self.create_sequence(mgr, clock_time, probe_time, laser_pause,  left=False)
    ...
    def create_sequence(self, mgr, odmr_span,sweep_time, clock_time, probe_time, laser_pause, n_points, left):
        ...
        self.n_points=sweep_time//(probe_time_ns*1e-9)
        sweep_time_ns=probe_time_ns*n_points
        f_values=np.linspace(0, odmr_span, n_points*number_bins)
        ...
        for f in f_values:
            phi+=2*np.pi*f*8e-9
            i_analog.append((8, np.cos(phi)))
            q_left.append((8, -np.sin(phi)))
            q_right.append((8, np.sin(phi)))
        i=[(laser_pause_ns+clock_time_ns+sweep_time_ns,0)]+i_analog+[(2*laser_pause_ns+2*clock_time_ns+sweep_time_ns,0)]+i_analog+[(laser_pause_ns+clock_time_ns,0)]
